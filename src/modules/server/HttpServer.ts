...
async function handleRequest(req: HttpRequest): Promise<{status: number; body: string}> {
  ...
  if (method === 'POST' && path === '/message') {
    ...
    messageStore.insertMessage({id: messageId, body: message, source: 'http'});
    // Fire-and-forget — response is returned immediately, sending happens in background
    sendSms({
      messageId,
      phoneNumbers,
      body: message,
    ...
    return {status: 200, body: JSON.stringify({id: messageId, status: 'queued'})};
  }
